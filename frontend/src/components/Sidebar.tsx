...
// Primary site navigation: page links plus the two modal triggers. Always
// visible on desktop; toggled by TopBar's "[메뉴]" button on mobile.
export function Sidebar({ currentPage, isOpen, onNavigate, onOpenQuickLinks, onOpenHelp }: SidebarProps) {
  return (
    <aside
      id="main-sidebar"
      aria-label="주요 메뉴"
    ...
    >
      ...
      <div className="mt-8 flex flex-col gap-3 border-t-4 border-ink pt-6">
        <button type="button" onClick={onOpenQuickLinks} className={navItemClass(false)}>
          자주 찾는 공공서비스
        </button>
        <button type="button" onClick={onOpenHelp} className={navItemClass(false)}>
          서비스 이용 방법
        </button>
      </div>
    </aside>
